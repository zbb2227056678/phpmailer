...
class Phpmailer {
     ...
     * Tries to use PHP built-in validator in the filter extension (from PHP 5.2), falls back to a reasonably competent
        ...
        try {
            ...
            if ($this->DKIM_domain && $this->DKIM_private) {
                $header_dkim = $this->DKIM_Add(
                    $this->MIMEHeader, $this->EncodeHeader($this->SecureHeader($this->Subject)), $this->MIMEBody
                );
        ...
        if ($this->Sender != '' and !ini_get('safe_mode')) {
            ...
            if ($this->SingleTo === true && count($toArr) > 1) {
                foreach ($toArr as $key => $val) {
                    $rt = @mail(
                        $val, $this->EncodeHeader($this->SecureHeader($this->Subject)), $body, $header, $params
                    );
                    // implement call back function if it exists
                    $isSent = ($rt == 1) ? 1 : 0;
                    $this->doCallback($isSent, $val, $this->cc, $this->bcc, $this->Subject, $body);
    ...
    public function CreateHeader() {
        ...
        if ($this->XMailer) {
            ...
            $result .= $this->HeaderLine(
                'X-Mailer', 'PHPMailer '.$this->Version.' (http://code.google.com/a/apache-extras.org/p/phpmailer/)'
            );
        }
    ...
    protected function AttachAll($disposition_type, $boundary) {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] == $disposition_type) {
                ...
                $mime[] = sprintf(
                    "Content-Disposition: %s; filename=\"%s\"%s", $disposition,
            ...
            foreach ($images[2] as $i => $url) {
                ...
                if (!preg_match('#^[A-z]+://#', $url)) {
                    ...
                    if ($this->AddEmbeddedImage(
                        $basedir.$directory.$filename, md5($filename), $filename, 'base64', $mimeType
                    )
                    ) {
